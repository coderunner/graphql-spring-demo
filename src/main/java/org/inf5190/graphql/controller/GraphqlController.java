...
public class GraphqlController {
    ...
    public List<Book> books(Author author) {
        return Arrays.asList(author.bookIds()).stream().map(bookId -> this.bookRepository.getById(bookId)).toList();
    }

    @SchemaMapping
    public List<Book> purchases(User user) {
        return Arrays.asList(user.purchasedBookIds()).stream().map(bookId -> this.bookRepository.getById(bookId))
                .toList();
    }
}